...
class TemporalAnalyzer:
    ...
    def analyze_best_posting_times(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        hourly_performance = df.groupby(['day_of_week', 'hour']).agg({
            'engagement': ['mean', 'count'], 'shortcode': 'count'
        }).reset_index()
        ...
        result = {"top_10_times": [], "by_day": {}, "recommendations": []}
        
        for _, row in best_times.iterrows():
            day_name = self.day_names.get(row['day_of_week'], 'Desconhecido')
            result["top_10_times"].append({
                "day": day_name, "day_index": int(row['day_of_week']),
                "hour": int(row['hour']), "formatted_time": f"{int(row['hour']):02d}:00",
    ...
    def _calculate_consistency_score(self, std_days: float, avg_days: float) -> float:
        ...
        if cv < 0.3: return 10.0
        elif cv < 0.5: return 8.0
        elif cv < 0.7: return 6.0
        elif cv < 1.0: return 4.0
        else: return 2.0
    ...
    def _get_frequency_recommendation(self, posts_per_week: float, std_days: float) -> str:
        if posts_per_week < 3:
            ...
            if std_days and std_days > 2:
                return "Mantenha a frequência mas melhore a consistência"
            ...
            return "Frequência muito alta - avalie burnout ou perda de qualidade"
    ...
    def analyze_growth_trend(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if len(df_sorted) > 5:
            ...
            if second_half > first_half * 1.2:
                trend, trend_score = "crescente", 8.0
            elif second_half > first_half * 1.05:
                trend, trend_score = "levemente_crescente", 6.5
            elif second_half > first_half * 0.95:
                trend, trend_score = "estavel", 5.0
            elif second_half > first_half * 0.8:
                trend, trend_score = "levemente_decrescente", 3.5
            else:
                trend, trend_score = "decrescente", 2.0
        else:
            trend, trend_score = "insuficiente_dados", 5.0
            first_half = second_half = None
        
        return {
            "trend": trend, "trend_score": trend_score,
            "first_half_avg_engagement": round(first_half, 2) if first_half else None,
            "second_half_avg_engagement": round(second_half, 2) if second_half else None,
            "change_percent": round((second_half - first_half) / first_half * 100, 2) if first_half and first_half > 0 else None
        }
    ...
    def run_full_analysis(self) -> Dict[str, Any]:
        ...
        if df is None:
            return {
                "success": False,
                "error": f"Dados insuficientes. Mínimo: {self.min_posts} posts com data.",
...
def main():
    ...
    parser.add_argument("--input", "-i", required=True, help="Caminho para JSON de posts")
    parser.add_argument("--output", "-o", default=None, help="Caminho para saída JSON")
    parser.add_argument("--min-posts", type=int, default=20, help="Mínimo de posts")
